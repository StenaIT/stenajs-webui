...
This makes it unsuitable for modals that are visible or hidden depending on state.
If you need state-driven visibility, `<Modal>` is a better fit.
...
```tsx
const { reject, resolve } = useDialogPromise<MealSelectionModalResult>();
