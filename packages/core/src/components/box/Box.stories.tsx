...
storiesOf('core/Box/Box', module)
  ...
  .add('row with justifyContent', () => (
    <Box row justifyContent={'flex-end'} width={'500px'} border={'1px solid grey'}>
      <div>hello</div>
      <div>world</div>
    </Box>
  ))
  ...
  .add('gradient background', () => (
    <Box background={`linear-gradient(to right, ${color(
      'Color 1',
      '#e66465',
    )}, ${color('Color 2', '#9198e5')})`} indent spacing>
      <div>hello</div>
      <div>world</div>
    </Box>
  ))
  .add('indent and spacing', () => (
    <Box background={'#DFCD59'} indent={number('Indent', 1, { range: true, min: 0, max: 10, step: 1 })}
  ...
  .add('custom shadow', () => (
    <Box shadow={`${color('Shadow color', 'red')} 0px 0px 10px 4px;`} indent spacing>
      <div>hello</div>
      <div>world</div>
    </Box>
  ))
